...
(function(){
  ...
  function isProductPage(p){
    if(/^.+-(brief|detailed)\.html$/.test(p) && (location.pathname || "").includes("/products/")) return true;
    if(document.querySelector(".product-layout, .angular-layout, [data-product-slug]")) return true;
    return p !== "product-guide.html" && ((document.title || "").toLowerCase().indexOf("bw product guide") !== -1);
  }

  function backTarget(p){
    var bodyBack = document.body && document.body.getAttribute("data-back");
    if(bodyBack) return bodyBack;
    ...
    if(p === "product-taxonomy-category.html") return "product-taxonomy.html";
    if(p === "product-taxonomy.html") return "resources.html";
    if(p === "rack-builder.html" || p === "rack-builder 2.html" || p === "order.html") return "resources.html";
    if(p === "supplier-directory.html" || p === "account-applications.html" || p === "crm1.html") return "resources.html";
    return "index.html";
  }
